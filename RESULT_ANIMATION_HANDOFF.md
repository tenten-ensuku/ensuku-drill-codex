...
- 各モードの神ランク基準は、ランキングの歴代1位と同じスコアにする。
- 歴代1位のスコアが更新されたときは、`RANKS` の該当モードとこの表を同じ点数へ更新して公開する。
